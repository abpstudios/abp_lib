...
local timers = {}
local cbEvent = '__abp_cb_%s'
local callbackTimeout = 300000

RegisterNetEvent(cbEvent:format(abpcache.resource), function(key, ...)
...
local function triggerServerCallback(_, event, delay, cb, ...)
    ...
    TriggerServerEvent(cbEvent:format(event), abpcache.resource, key, ...)
...
abplib.callback = setmetatable({}, {
    __call = function(_, event, delay, cb, ...)
        if not cb then
            warn(("callback event '%s' without function to callback to and will instead await\nuse abplib.callback.await or a regular event to remove this warning")
                :format(event))
        else
...
function abplib.callback.await(event, delay, ...)
...
---@param cb function
---@diagnostic disable-next-line: duplicate-set-field
function abplib.callback.register(name, cb)
    RegisterNetEvent(cbEvent:format(name), function(resource, key, ...)
        TriggerServerEvent(cbEvent:format(resource), key, callbackResponse(pcall(cb, ...)))
    end)
end

return abplib.callback